...
def model_router(model):
    ...
    route_key = model or "default"
    env_override = _env_override(route_key)

    use_default_route = model in (None, "", "default")
    use_benchmark_route = model in ("benchmark", "eval", "evaluation")

    if use_default_route:
        entry = default
        fallback_provider = default.get("provider") or "openai"
        fallback_model_id = default.get("model_id") or "gpt-5.4"
        base_url_default = default
    elif use_benchmark_route:
        entry = benchmark or routes.get("gpt-5.4", {}) or routes.get("gpt-5.2", {})
        fallback_provider = entry.get("provider") or default.get("provider") or "openai"
        fallback_model_id = entry.get("model_id") or "gpt-5.4"
        base_url_default = default
    else:
        entry = routes.get(model, {}) if isinstance(routes, dict) else {}
        fallback_provider = "openai"
        fallback_model_id = str(model or "gpt-5.4")
        base_url_default = {}
    ...
    base_url = _select_base_url(entry, base_url_default)
